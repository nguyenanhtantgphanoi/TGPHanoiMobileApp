...
export default function WebViewDetail({ linkWeb }) {
    ...
    const getReaderCSS = (fSize, tName) => `
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; text-decoration: none !important; }
        ...
        body { 
            padding: 0 20px; /* Padding này để giữ nội dung chữ không dính lề */
        }
        h1 { font-size: ${fSize + 4}px; margin: 25px 0; font-weight: 700; text-align: center; }
        
        /* FIX CHUẨN: Ảnh tràn 100% màn hình, không bị lệch trái, không hở phải */
        img, figure { 
            display: block !important;
            height: auto !important;
            width: 100vw !important;
            max-width: none !important; /* Quan trọng: Gỡ bỏ giới hạn của thẻ cha */
            
            /* Công thức thần thánh để căn giữa ảnh 100vw vào giữa body có padding */
            position: relative;
            left: 50%;
            right: 50%;
        ...
        figure { margin-top: 20px; margin-bottom: 20px; padding: 0; }
    ...
    const activateReaderScript = `
        (function() {
            ...
            try {
                ...
                const junk = [
                    'script', 'style', '.ads', '.social-share', '.sfsi_responsive_icons', 
                    '.sfsi_icons_container', '.fb-like', '.share-buttons', 'footer', 'nav',
                    '[class*="facebook"]', '[class*="twitter"]', '[class*="share"]'
                ];
                ...
                const readerHTML = \`
                    ...
                    <head>
                        ...
                        <style id="reader-style">\${${JSON.stringify(getReaderCSS(fontSize, theme))}}</style>
                    </head>
                    <body><h1>\${title}</h1><div class="content">\${contentNode.innerHTML}</div><div style="height:100px;"></div></body>
                    </html>\`;
    ...
    return (
        <View style={[styles.container, { backgroundColor: readerMode ? THEMES[theme].bg : '#fff' }]}>
            ...
            <Modal visible={showSettings} transparent animationType="slide">
                <View style={styles.modalOverlay}><View style={styles.panel}>
